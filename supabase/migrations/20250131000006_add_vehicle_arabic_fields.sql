-- Add additional fields for vehicle details form
ALTER TABLE public.vehicles
ADD COLUMN IF NOT EXISTS actual_user_name VARCHAR(255),
ADD COLUMN IF NOT EXISTS owner_id_number VARCHAR(255),
ADD COLUMN IF NOT EXISTS actual_user_id_number VARCHAR(255),
...
CREATE INDEX IF NOT EXISTS idx_vehicles_owner_id_number ON public.vehicles(owner_id_number);
CREATE INDEX IF NOT EXISTS idx_vehicles_actual_user_id_number ON public.vehicles(actual_user_id_number);
...
COMMENT ON COLUMN public.vehicles.owner_id_number IS 'Owner ID number';
COMMENT ON COLUMN public.vehicles.actual_user_id_number IS 'User ID number';
