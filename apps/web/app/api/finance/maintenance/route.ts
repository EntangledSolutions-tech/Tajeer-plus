...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get transaction type ID for "Maintenance" category
    ...
    const { data: transaction, error: transactionError } = await supabase
      ...
      .insert({
        ...
        description: notes || 'Vehicle maintenance record',
    ...
    if (transactionError) {
      console.error('Error creating maintenance transaction:', transactionError);
      return NextResponse.json({
        error: 'Failed to create maintenance transaction'
      }, { status: 500 });
    }
    ...
    const { error: expenseError } = await supabase
      ...
      .insert({
        ...
        expense_type: 'Vehicle Maintenance',
