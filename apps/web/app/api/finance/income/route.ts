...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Income API received:', body);
    ...
    const { error: incomeError } = await (supabase as any)
      ...
      .insert({
        ...
        income_type: transactionTypeData.name.toLowerCase().replace(/\s+/g, '_'),
        source: 'contract'
      });
...
export async function GET(request: NextRequest) {
  try {
    ...
    // First get transaction type IDs for income category
    const { data: incomeTransactionTypes, error: typeError } = await (supabase as any)
      .from('finance_transaction_types')
      .select('id')
      .eq('category', 'income')
      .eq('is_active', true);
    ...
    if (typeError) {
      console.error('Error fetching income transaction types:', typeError);
      return NextResponse.json({ error: 'Failed to fetch transaction types' }, { status: 500 });
    }

    const incomeTypeIds = incomeTransactionTypes?.map((t: any) => t.id) || [];
    ...
    let query = (supabase as any)
      ...
      `, { count: 'exact' })
      .eq('user_id', user.id)
      .in('transaction_type_id', incomeTypeIds);
