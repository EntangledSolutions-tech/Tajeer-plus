...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Accident API received:', body);
    ...
    // Get transaction type ID for "Accident" category
    ...
    const { data: transaction, error: transactionError } = await supabase
      ...
      .insert({
        ...
        description: notes || 'Vehicle accident record',
    ...
    if (transactionError) {
      console.error('Error creating accident transaction:', transactionError);
      return NextResponse.json({
        error: 'Failed to create accident transaction'
      }, { status: 500 });
    }
    ...
    const { error: expenseError } = await supabase
      ...
      .insert({
        ...
        expense_type: 'Vehicle Accident',
        vendor_name: body.vendor || 'Repair Shop',
    ...
    return NextResponse.json({
      ...
      message: 'Vehicle accident record created successfully'
    });

  } catch (error) {
    console.error('Error in accident API:', error);
    return NextResponse.json({
