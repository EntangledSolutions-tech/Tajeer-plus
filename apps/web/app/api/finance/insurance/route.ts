...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Insurance API received:', body);
    ...
    // Get transaction type ID for "Insurance" category
    ...
    const { data: transaction, error: transactionError } = await supabase
      ...
      .insert({
        ...
        description: notes || 'Vehicle insurance payment',
    ...
    if (transactionError) {
      console.error('Error creating insurance transaction:', transactionError);
      return NextResponse.json({
        error: 'Failed to create insurance transaction'
      }, { status: 500 });
    }
    ...
    const { error: expenseError } = await supabase
      ...
      .insert({
        ...
        expense_type: 'Vehicle Insurance',
        vendor_name: body.vendor || 'Insurance Company',
    ...
    return NextResponse.json({
      ...
      message: 'Vehicle insurance payment created successfully'
    });
