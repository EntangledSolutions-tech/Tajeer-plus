...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Service API received:', body);
    ...
    const { vehicle, amount, date, description } = body;
    if (!vehicle || !amount || !date) {
      return NextResponse.json({
        error: 'Missing required fields: vehicle, amount, date'
      }, { status: 400 });
    }

    // Get transaction type ID for "Service" category
    ...
    const { data: transaction, error: transactionError } = await supabase
      ...
      .insert({
        ...
        description: description || 'Vehicle service record',
    ...
    if (transactionError) {
      ...
      return NextResponse.json({
        error: 'Failed to create service transaction'
      }, { status: 500 });
    }
    ...
    const { error: expenseError } = await supabase
      ...
      .insert({
        ...
        expense_type: 'Vehicle Service',
        vendor_name: body.vendor || 'Service Center',
